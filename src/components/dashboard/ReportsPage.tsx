...
import { Search, Users, Package, DollarSign, Download, Calendar as CalendarIcon, TrendingUp, ShoppingCart, Tag } from "lucide-react";
...
export const ReportsPage = ({ storeId }: ReportsPageProps) => {
  ...
  const [allOrders, setAllOrders] = useState<OrderReport[]>([]);
  ...
  const fetchAllOrders = async () => {
    try {
      ...
      // Mapear dados para garantir compatibilidade com a interface
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Lista completa de todos os pedidos com detalhes
            </p>
          </div>
          <Button
            variant="outline"
            size="sm"
            onClick={() => exportToCSV(filteredOrders, 'relatorio_pedidos')}
          >
            <Download className="h-4 w-4 mr-2" />
            Exportar
          </Button>
