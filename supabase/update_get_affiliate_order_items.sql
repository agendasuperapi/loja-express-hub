-- Atualizar função get_affiliate_order_items para retornar commission_source correto

CREATE OR REPLACE FUNCTION public.get_affiliate_order_items(p_order_id uuid, p_store_affiliate_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(item_id uuid, product_id uuid, product_name text, product_category text, quantity integer, unit_price numeric, subtotal numeric, item_discount numeric, item_value_with_discount numeric, is_coupon_eligible boolean, coupon_scope text, commission_type text, commission_source text, commission_value numeric, item_commission numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_has_item_earnings THEN
    ...
    SELECT 
      ...
      -- Retorna commission_source salvo no banco, com fallback para registros antigos
  ...
  ELSE
    ...
    SELECT 
      ...
      'proporcional'::TEXT as commission_source,  -- Indica que é cálculo proporcional
