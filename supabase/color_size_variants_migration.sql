...
-- Creates tables and policies for linking colors to sizes with availability control
...
-- Create order_item_variants table for historical data
...
COMMENT ON TABLE public.color_size_variants IS 'Matrix linking colors and sizes for products with availability control';
COMMENT ON COLUMN public.color_size_variants.stock_quantity IS 'Optional stock control per variant combination';
COMMENT ON COLUMN public.color_size_variants.price_adjustment IS 'Additional price adjustment for this specific combination (on top of individual color/size adjustments)';
COMMENT ON TABLE public.order_item_variants IS 'Historical record of color-size variants selected in orders';
