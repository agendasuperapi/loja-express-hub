-- ============================================================
-- FIX: Corrigir timing da comissão de afiliados
-- O trigger anterior executava APÓS INSERT em orders,
-- mas ANTES dos order_items serem inseridos, resultando em 
-- comissões zeradas. Esta correção move a lógica para executar
-- APÓS os itens serem inseridos.
-- ============================================================

-- 1. Desabilitar triggers antigos em orders (para não duplicar)
...
-- 2. Criar função para processar comissão que pode ser chamada manualmente
CREATE OR REPLACE FUNCTION process_affiliate_commission_for_order(p_order_id UUID)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
...
-- 3. Criar função trigger para order_items
CREATE OR REPLACE FUNCTION trigger_process_commission_after_items()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DECLARE
  v_item_count INT;
BEGIN
  -- Contar quantos itens existem para este pedido
  SELECT COUNT(*) INTO v_item_count 
  FROM order_items 
  WHERE order_id = NEW.order_id AND deleted_at IS NULL;
  
  -- Só processar no primeiro item para evitar múltiplas execuções
  IF v_item_count = 1 THEN
    -- Usar pg_sleep_for para dar tempo dos outros itens serem inseridos
    -- (alternativa: usar deferred trigger ou job queue)
...
-- 4. Criar trigger em order_items
...
CREATE TRIGGER process_commission_after_items_trigger
  ...
  FOR EACH ROW
  EXECUTE FUNCTION trigger_process_commission_after_items();

-- 5. Conceder permissões
GRANT EXECUTE ON FUNCTION process_affiliate_commission_for_order(UUID) TO authenticated, anon, service_role;
GRANT EXECUTE ON FUNCTION trigger_process_commission_after_items() TO authenticated, anon, service_role;

-- 6. Comentários
COMMENT ON FUNCTION process_affiliate_commission_for_order IS 'Processa comissão de afiliado para um pedido específico. Chamada após os itens serem inseridos.';
COMMENT ON FUNCTION trigger_process_commission_after_items IS 'Trigger que dispara o processamento de comissão após inserir itens do pedido.';
