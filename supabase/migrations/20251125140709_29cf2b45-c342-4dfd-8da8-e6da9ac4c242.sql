-- Modificar a função notify_order_whatsapp para verificar uma flag de sessão
...
BEGIN
  -- Verificar se há uma flag de sessão para pular notificação
  BEGIN
    skip_notification := current_setting('app.skip_whatsapp_notification', TRUE);
  EXCEPTION WHEN OTHERS THEN
    skip_notification := NULL;
  END;

  -- Se a flag estiver configurada para 'true', pular notificação
  IF skip_notification = 'true' THEN
    RAISE NOTICE 'Skipping WhatsApp notification due to session flag';
    RETURN NEW;
  END IF;
